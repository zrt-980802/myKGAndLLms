...
def main():
    ...
    for entry in goldData["Questions"]:
        ...
        if id not in PredAnswersById:
            print(
            "The problem " + id + " is not in the prediction set")
            print(
        ...
        if len(entry["Parses"]) == 0:
            print(
                "Empty parses in the gold set. Breaking!!")
            break
    ...
    print(
        "Number of questions:", int(total))
    print(
        "Average precision over questions: %.3f" % (precSum / total))
    print(
        "Average recall over questions: %.3f" % (recSum / total))
    print(
        "Average f1 over questions (accuracy): %.3f" % (f1sum / total))
    print(
        "F1 of average recall and average precision: %.3f" % (
                2 * (recSum / total) * (precSum / total) / (recSum / total + precSum / total)))
    print(
        "True accuracy (ratio of questions answered exactly correctly): %.3f" % (numCorrect / total))
